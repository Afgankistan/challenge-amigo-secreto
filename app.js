...
function agregarAmigo() {// funcion agregar amigo
    const nombreAmigo = document.getElementById('amigo').value.trim(); // obtener el valor del input y trim() para eliminar espacios al inicio/final
...
function sortearAmigo() {//  funcion de sortear amigos 
    ...
    document.getElementById('resultado').innerHTML = `<li>🎉 El amigo secreto es: <strong>${ganador}</strong></li>`;// mensaje del sorteo
    setTimeout(() => { //se establece un tiempo de 7 segundos para reiniciar el juego
            reiniciarJuego();
        }, 7000);
}
